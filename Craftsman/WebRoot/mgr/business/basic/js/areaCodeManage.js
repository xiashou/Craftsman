...
Ext.onReady(function() {
	...
	var areaCodeForm = new Ext.form.FormPanel({
				id : 'areaCodeForm',
				layout: 'anchor',
				defaults: {
		            anchor: '100%',
				...
				items : [
					{
					    ...
					    items: [
					           {id:'id', name:'areaCode.id', xtype:'hiddenfield'}
					    ]
					},{
					    defaults: {flex: 1, xtype:'textfield', labelWidth:70, labelAlign:'right'},
					    items: [
					           {id:'areaCode', name: 'areaCode.areaCode', allowBlank: false, fieldLabel: '区域代码', blankText:'代码不能为空!' }
					    ]
					}
				],
				buttons:[{
					...
					handler : function() { // 按钮响应函数
						if (areaCodeForm.form.isValid()) {
							areaCodeForm.form.submit({
								waitTitle : '提示',
								method : 'POST',
								waitMsg : '正在处理数据,请稍候...',
	...
	var areaCodeWindow = new Ext.Window({
				title : '修改', // 窗口标题
				layout : 'fit',
				width : 400,
				height : 150,
	...
	var store = Ext.create('Ext.data.Store', {
		          ...
		          proxy: {
		              type: 'ajax',
		              url : '/base/queryAreaCodePage.atc',
		              reader : {
		                  root: 'areaCodeList',
  	...
  	var columns= [rowNum,
	            ...
	            {header : '代码',dataIndex: 'areaCode', width: 200}
	  	]; 
	...
	var tbar = Ext.create('Ext.toolbar.Toolbar', {
	        ...
	        items: [{
	        	...
	        	handler: function() {
	        		areaCodeWindow.setTitle('新建');
	        		areaCodeForm.getForm().reset();
	        		areaCodeForm.getForm().url = '/base/insertBaseArea.atc';
	        		areaCodeWindow.show();
	            }
	        },{
	        	xtype: 'button',
...
//	            }
	...
	function initEdit() {
		...
		areaCodeWindow.setTitle('修改');
		areaCodeForm.loadRecord(record);
		areaCodeForm.getForm().url = '/sys/updateSysButton.atc';
		areaCodeWindow.show();
	}
