...
Ext.onReady(function() {
	...
	//订单类型5保险理赔, 显示应收, 实际应收, 实收, 库存报警值, 积分抵现值, 抵现金额, 积分规则
	var orderType = "5", receivable = 0, rece = 0, actual = 0, stockWarn = 0, offsetParam = 0, poffset = 0, pointRule = 0;	
	...
	var OrderHead = Ext.create('Ext.form.FormPanel', {
		...
		items : [{
        	...
        	margin: '0 0 5 5',
        	defaults:{
            	flex: 1, xtype: 'textfield', padding: '0 5 0 0', labelWidth: 60, labelAlign: 'right',
            },
            items:[{
            	xtype: 'displayfield',
            	minWidth: 120
	...
	var ProjectSecondItemRenderer = function(value, cellmeta, record) {
        var index = ProjectSecondItemStore.find("id", value);
        var record = ProjectSecondItemStore.getAt(index);
        var displayText = "";
        if (record == null)
        	displayText = value;
        else
	...
	var ProjectColumns= [ProjectRowNum,
	        ...
	        {header : '项目销售方',dataIndex: 'seller',menuDisabled: true, renderer: EmployeeRenderer,
		    	editor: {xtype: 'textfield', allowBlank: false,
                    listeners: {
                    	focus: function(field, newValue, oldValue){
                    		PerformerWindow.hide();
...
//            { xtype: 'button', iconCls:'table_relationship', text: '绑定微信' },
	...
	function submitOrder(deposit){
		if(Number(actual) < Number(receivable) && Ext.isEmpty(deposit)){
			...
			return false;
		} else if(Ext.isEmpty(Ext.getCmp("inscompany").getValue())) {
			Ext.MessageBox.show({
	           title: '提示',
	           msg: "保险公司不能为空！",
	           buttons: Ext.MessageBox.OK,
			...
			Ext.getCmp("inscompany").focus();
			return false;
		} else if(Ext.isEmpty(Ext.getCmp("sender").getValue())) {
			Ext.MessageBox.show({
				title: '提示',
				msg: "送修人不能为空！",
				buttons: Ext.MessageBox.OK,
			...
			Ext.getCmp("sender").focus();
			return false;
		} else if(Ext.isEmpty(Ext.getCmp("commission").getValue())) {
			Ext.MessageBox.show({
				title: '提示',
				msg: "提成比率不能为空！",
				buttons: Ext.MessageBox.OK,
...
//        }
        ...
        Ext.Ajax.request({
            ...
            params : {
            	...
            	'orderHead.inscompany' : Ext.getCmp("inscompany").getValue(),
            	'orderHead.sender' : Ext.getCmp("sender").getValue(),
            	'orderHead.commission' : Ext.getCmp("commission").getValue(),
