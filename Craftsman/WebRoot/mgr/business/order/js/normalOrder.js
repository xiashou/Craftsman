...
Ext.onReady(function() {
	...
	//订单类型1普通订单, 显示应收, 实际应收, 实收, 库存报警值, 积分抵现值, 抵现金额, 积分规则
	var orderType = "1", receivable = 0, rece = 0, actual = 0, stockWarn = 0, offsetParam = 0, poffset = 0, pointRule = 0;	
	...
	var CarForm = Ext.create('Ext.form.Panel', {
		...
		items : [{
		         ...
		         items: [
	                    ...
	                    {id:'number', name: 'car.carNumber', maxLength: 6, flex: 4, allowBlank: false, blankText:'车牌号号码不能为空!'}
		         ]
		     },{
		         items: [BrandCombo]
		     },{
	...
	var HistorySalesColumns= [HistorySalesRowNum,
	            {header : '订单号',dataIndex: 'orderId',menuDisabled: true},
	 	        ...
	 	        {header : '次数/数量',dataIndex: 'number',menuDisabled: true,summaryType: 'sum'},
	 	        {header : '价格',dataIndex: 'price',menuDisabled: true,renderer: Ext.util.Format.numberRenderer("0.00"),summaryType: 'sum'}
	 	];
    ...
    var SellerPanel = Ext.create('Ext.form.Panel', {
        ...
        buttons:[{
			...
			handler : function() {
                ...
                if(choiceGoodsWindow.isHidden()){
                	var rowRecord = ProductGrid.getSelectionModel().getLastSelected();
                	rowRecord.data.seller = value;
                	rowRecord.commit();
                	SellerWindow.hide();
                } else {
                	Ext.getCmp("choiceSeller").setValue(value);
                	Ext.getCmp("choiceSellerDisplay").setValue(display);
                	SellerWindow.hide();
                }
			}
        }]
...
//                	insertProject();
            ...
            change: function(combox, newValue, oldValue){
            	if(!Ext.isEmpty(ProjectItemCombo.getValue())){
            		var rowRecord = ProjectGrid.getSelectionModel().getLastSelected();
            		...
            		if(Ext.isEmpty(rec) && rowRecord){
            			...
            			rowRecord.data.goodsId = '00000000';
            			rowRecord.data.goodsName = newValue;
            			rowRecord.commit();
            		}
            	} else
            		Ext.Msg.alert('提示', "请先添加大项!");
        	}
        }
	});
...
//		fields: ['id', 'code', 'name', 'number', 'price', 'color', 'suitModel'],
//		proxy: {
//            type: 'ajax',
//            url : '/goods/queryGoodsMaterialStockByKeyword.atc',
//            reader : {
...
//            getInnerTpl: function() {
//                return '<div class="search">{code}/{name}/{price}/{color}/{suitModel}/ 库存:{number}件</div>';
//            }
//        },
//        listeners : {
//        	select: function(cb, records) {
...
//			},
//			blur: function(cb){
//				if(Ext.isEmpty(cb.getValue()) || isNaN(cb.getValue())){
//					Ext.Msg.alert('提示', "产品不存在!");
//				}
//			}
//		}
//	});
...
//    };
	var ProductColumns= [ProductRowNum,
	        ...
	        {header : '单价（元）',dataIndex: 'unitPrice',menuDisabled: true,renderer: Ext.util.Format.numberRenderer("0.00"), editor: {xtype: 'numberfield', allowBlank: false,
	        	listeners: {
                	change: function(field, newValue, oldValue){
                		var rowRecord = ProductGrid.getSelectionModel().getLastSelected();
...
//                				Ext.example.msg("提示", poffset + "|" + (receivable - poffset));
...
//        var giveProductArray = [];	//商品赠送
        ...
        Ext.each(ProductStore.data.items, function(item){
        	if(isNaN(item.data.goodsId))
        		errorMsg += item.data.goodsId + ' ';
        	if(item.data.seller == '请选择')
    			item.data.seller = '';
        	if(!Ext.isEmpty(item.data.goodsId))
