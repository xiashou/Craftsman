...
//		plugins: [ProjectCellEditing],
    ...
    var ProductRenderer = function(value, cellmeta, record) {
    	if(!Ext.isEmpty(record) && !Ext.isEmpty(record.data.goodsName))
    		return record.data.goodsName;
    	else {
    		var index = StockStore.find("id", value);
            var rec = StockStore.getAt(index);
            var displayText = "";
            if (rec == null)
            	displayText = value;
            else {
            	if(Number(rec.data.number) > stockWarn)
            		displayText = rec.data.code + "/" + rec.data.name;
            	else {
            		//库存报警，字体红色，播放音乐
            		displayText = "<font color=red>" + rec.data.code + "/" + rec.data.name + "（库存" + rec.data.number + "件）</font>";
		...
		Ext.Ajax.request({
			...
			success : function(resp, opts) {
				...
				if(orderHead && !Ext.isEmpty(orderHead)){
					...
					Ext.getCmp("orderRemark").setValue(orderHead.remark);
...
//							completeMemberService(record.memId);
	...
	function submitOrder(type){
		if(Number(actual) < Number(receivable) && type == 1){
			...
			CarCombo.focus();
			return false;
		} else if(Number(Ext.getCmp("pbalance").getValue()) > Number(memberRecord.data.balance) && type == 1){
			Ext.MessageBox.show({
	           title: '提示',
	           msg: "余额不足！",
	           buttons: Ext.MessageBox.OK,
