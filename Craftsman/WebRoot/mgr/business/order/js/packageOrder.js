...
Ext.onReady(function() {
	...
	//订单类型2套餐购买, 显示应收, 实际应收, 实收, 库存报警值, 积分抵现值, 抵现金额, 积分规则
	var orderType = "2", receivable = 0, rece = 0, actual = 0, stockWarn = 0, offsetParam = 0, poffset = 0, pointRule = 0;	
	...
	var CarStore = Ext.create('Ext.data.Store', {
		fields : ['id', 'memberId', 'carShort', 'carCode', 'carNumber', 'carBrand', 'carSeries', 'carModel', 'carFrame', 'showName', 
		          'carEngine', 'carKilometers', 'carNextkilo', 'carInspection', 'carMaintain', 'carInsurance', 'carInsuCompany', 'cariLLegal'],  
		proxy:{
            type: 'ajax',
        	...
        	items: [{
				...
				items: [{
				    ...
				    menu: [
				           ...
				           {text: '客户充值', value:'recharge', handler: OrderTypeChange}
				    ]
				}]
	...
	var PackageStore = Ext.create('Ext.data.Store', {
		autoLoad: true,
		fields: ['id', 'name','price','expire','startDate','endDate','explain','status','createTime'],
		proxy:{
            type: 'ajax',
	...
	var PackageColumns= [PackageRowNum,
	        ...
	        {header : '套餐名称',dataIndex: 'name',menuDisabled: true, width: '22%'},
	        {header : '价格',dataIndex: 'price',menuDisabled: true, renderer: Ext.util.Format.numberRenderer("0.00")},
	        {header : '有效期',dataIndex: 'expire',menuDisabled: true},
	        {header : '开始日期',dataIndex: 'startDate',menuDisabled: true, hidden:true},
	        {header : '结束日期',dataIndex: 'endDate',menuDisabled: true, hidden:true}
	];
	var PackageSelModel = Ext.create('Ext.selection.CheckboxModel', {
		injectCheckbox: 1
//		mode: 'SINGLE'
	});
	var PackageGrid = Ext.create('Ext.grid.GridPanel', {
		region: 'center',
		title: '套餐概览',
	...
	var PackageDetailStore = Ext.create('Ext.data.Store', {
		  fields: ['gpId', 'itemNo', 'goodsId', 'number', 'goodsName'],
		  proxy: {
		      type: 'ajax',
	...
	var PackageDetailColumns= [PackageDetailRowNum,
	        {header : 'ID', dataIndex: 'gpId', hidden: true},
	        {header : '商品ID', dataIndex: 'goodsId',hidden: true},
	        {header : '商品名称', dataIndex: 'goodsName', menuDisabled: true},
                	...
                	change: function(field, newValue, oldValue){
                		if(field.isValid()){
                			...
                			if(Number(newValue) <= point){
                				...
                				oldPoffset = (Number(oldValue) / Number(offsetParam)).toFixed(2);
                				if(Number(poffset) > (Number(receivable) + Number(oldPoffset))){
					...
					listeners: {
	                	change: function(field, newValue, oldValue){
	                		if(!isNaN(field.value)) {
	                			receivable = newValue;
	                			if(memberRecord){
	                				if(Number(receivable) >= Number(poffset) && Number(memberRecord.data.point) > 0)
...
//            { xtype: 'button', iconCls:'table_relationship', text: '绑定微信' },
	...
	//套餐信息
	var PackageInfo = Ext.create('Ext.panel.Panel',{
		region: 'center',
        id: 'PackageInfo',
        border: false,
        autoScroll: false,
        animCollapse: true,
        layout:'border',
        items:[PackageGrid, PackageDetailGrid]
	});
	...
	var OrderInfo = Ext.create('Ext.panel.Panel',{
        ...
        items:[OrderHead, PackageInfo, OrderSubmit]
	});
	...
	function completeReceivable(){
		receivable = 0
		var packageArray = PackageSelModel.getSelection();
		Ext.each(packageArray, function(item){
			receivable += Number(item.data.price);
        });
	...
	function submitOrder(deposit){
		if(Number(actual) < Number(receivable) && Ext.isEmpty(deposit)){
			Ext.MessageBox.show({
		           title: '提示',
		           msg: "实收金额不能小于应收金额！",
		           buttons: Ext.MessageBox.OK,
			...
			CarCombo.focus();
			return false;
		} else if(Number(Ext.getCmp("pbalance").getValue()) > 0 && Number(Ext.getCmp("pbalance").getValue()) > Number(memberRecord.data.balance)){
			Ext.MessageBox.show({
	           title: '提示',
	           msg: "余额不足！",
	           buttons: Ext.MessageBox.OK,
        ...
        Ext.each(PackageSelModel.getSelection(), function(item){
        	if(!Ext.isEmpty(item.data.id)){
        		if(Ext.isEmpty(item.data.expire))
        			item.data.expire = 0;
        		packageArray.push(item.data);
        	}
        });
        if(Ext.isEmpty(packageArray) && Ext.isEmpty(packageArray)){
        	Ext.MessageBox.show({
		           title: '提示',
		           msg: "没有任何消费项目！",
		           buttons: Ext.MessageBox.OK,
        ...
        Ext.Ajax.request({
            method:'POST',
            timout: 600000,
            url: '/order/submitPackageOrder.atc',
            params : {
            	...
            	'orderHead.aprice' : Ext.isEmpty(deposit) ? Ext.getCmp("actual").getValue() : deposit,
            	'orderHead.point' : completeSalePoint(),
            	'orderHead.poffset' : Number(Ext.getCmp("deductible").getValue()),
            	'orderHead.pdeposit' : Ext.isEmpty(deposit) ? 0 : deposit,		//定金
            	...
            	packageString : Ext.encode(packageArray),
