Ext.onReady(function() {
	...
	var store = Ext.create('Ext.data.Store', {
		...
		proxy : {
			type : 'ajax',
			url : '/wechat/act/queryActBannerList.atc',
			reader : {
				root : 'bannerList'
			}
		}
	...
	var logoRenderer = function(value, metaData, record) {
		if(value){
  			metaData.tdAttr = "data-qtip=\"<img src='/upload/wechat/activity/banner/" + value + "' style='width:320px; height:150px'/>\""; 
  			return '<img src="/upload/wechat/activity/banner/' + value + '" style="height:60px" onerror="this.src=\'/resources/images/noImage.jpg\'" />';
  		} else
		...
		items : [ {
			...
			handler : function() {
				if(store.getCount() == 3){
					...
					mallBannerForm.getForm().url = '/wechat/act/insertActBanner.atc';
	...
	function initEdit() {
		...
		mallBannerForm.getForm().url = '/wechat/act/updateActBanner.atc';
		mallBannerWindow.show();
	}
	...
	function initDelete() {
		...
		Ext.Msg.confirm('请确认', '确定要删除这张图片吗?', function(btn, text) {
			if (btn == 'yes') {
				Ext.Ajax.request({
					url : '/wechat/act/deleteActBanner.atc',
					params : {
