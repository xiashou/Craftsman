...
public class OAuth2 extends WeixinSupport implements java.io.Serializable {
	private static Logger wechatSysLog = Logger.getLogger("wechatSysLog");
    ...
    public String getOAuth2CodeUrl(String appId, String returnUrl, String scope, String state) {
    	if (Configuration.isDebug()) 
    		wechatSysLog.warn("-------------- 微信鉴权获取CODE开始----------------");
    	
    	String componentAppId = ConfigurationUtil.getSystemAppid();//服务方的appid，在申请创建公众号服务成功后，可在公众号服务详情页找到
    	
        try {
        	String codeUrl = authorize_url + "?appid=" + appId + "&redirect_uri=" + URLEncoder.encode(returnUrl, "UTF-8") 
            + "&response_type=code&scope=" + scope + "&state=" + state 
            + "&component_appid=" + componentAppId + "#wechat_redirect";
        	
        	if (Configuration.isDebug()) {
        		wechatSysLog.warn("-------------- 微信鉴权获取RETURN: " + returnUrl + "----------------");
        		wechatSysLog.warn("-------------- 微信鉴权获取CODEURL: " + codeUrl + "----------------");
        	}
        	...
        	if (Configuration.isDebug()) 
        		wechatSysLog.warn("-------------- 微信鉴权获取CODE结束----------------");
		}
    }
     ...
     */
    public OAuth2Token login(String appId, String componentAccessToken, String code) throws Exception {
        return login(appId, componentAccessToken, code, "authorization_code");
    }
     ...
     * @since Weixin4J 1.0.0
     */
    public OAuth2Token login(String appId, String componentAccessToken, String code, String grantType) throws Exception {
    	if (Configuration.isDebug()) 
    		wechatSysLog.warn("-------------- 微信鉴权开始appId：" + appId + "code：" + code + "----------------");
    	
        if (appId == null || componentAccessToken == null || appId.equals("") || componentAccessToken.equals("") || code == null || code.equals("") || grantType == null || grantType.equals("")) {
            throw new WeixinException("invalid null, appid or componentAccessToken or code is null.");
        }
        //设置code
        ...
        if (this.oauth != null && this.oauth2Token != null) {
            ...
            if (!oauth2Token.isExprexpired()) {
                ...
                if (this.oauth.getAppId().equals(appId) && this.oauth.getSecret().equals(componentAccessToken)) {
                    //如果没有过期，则直接返回对象
                    return oauth2Token;
                }
            }
        ...
        //拼接参数
        String componentAppId = ConfigurationUtil.getSystemAppid();//服务方的appid，在申请创建公众号服务成功后，可在公众号服务详情页找到
        String param = "?grant_type=" + grantType + "&appid=" + appId + "&code=" + code
        		+ "&component_appid=" + componentAppId + "&component_access_token=" + componentAccessToken;
        
        if (Configuration.isDebug()) 
    		wechatSysLog.warn("-------------- 微信鉴权参数：" + param + "----------------");
        
        //创建请求对象
        HttpsClient http = new HttpsClient();
        //调用获取access_token接口
        Response res = http.get("https://api.weixin.qq.com/sns/oauth2/component/access_token" + param);
        //根据请求结果判定，是否验证成功
        ...
        if (jsonObj != null) {
            ...
            if (obj != null) {
                //设置公众号信息
                oauth = new OAuth(appId, componentAccessToken, "");
                //设置凭证
        ...
        if (Configuration.isDebug()) 
    		wechatSysLog.warn("-------------- 微信鉴权结束，JSONObject：" + jsonObj.toString() + "----------------");
     ...
     * @throws Exception 
     */
    public OAuth2Token refreshToken() throws Exception {
        //必须先调用检查登录方法
        checkToken();
        //拼接参数
        String componentAppId = ConfigurationUtil.getSystemAppid();//服务方的appid，在申请创建公众号服务成功后，可在公众号服务详情页找到
        String param = "?appid=" + this.oauth.getAppId() + "&refresh_token=" + this.oauth2Token.getRefresh_token() + "&grant_type=refresh_token"
        		 + "&component_appid=" + componentAppId + "&component_access_token=" + this.oauth.getSecret();
        //创建请求对象
        HttpsClient http = new HttpsClient();
        //调用获取access_token接口
        Response res = http.get("https://api.weixin.qq.com/sns/oauth2/component/refresh_token" + param);
        //根据请求结果判定，是否验证成功
        JSONObject jsonObj = res.asJSONObject();
        if (jsonObj != null) {
            if (Configuration.isDebug()) {
     ...
     * @throws Exception 
     */
    public OAuth2User getUserInfo() throws Exception {
        //默认简体中文
     ...
     * @throws Exception 
     */
    public OAuth2User getUserInfo(String lang) throws Exception {
        //必须先调用检查登录方法
        checkToken();
