...
public class DefaultMessageHandler implements IMessageHandler {
    ...
    @Override
    public String invoke(ServletInputStream inputStream, String authorizerAppId) throws WeixinException {
        try {
            //将输入流转换为字符串
            ...
            return this.invoke(xmlMsg, authorizerAppId);
    ...
    @Override
    public String invoke(String inputXml, String authorizerAppId) throws WeixinException {
        //输出消息对象
        ...
        try {
            ...
            inputMsg.setAppId(authorizerAppId);
            
            //处理全网发布信息
            String caseAppId = "wx570bc396a51b8ff8";//全网发布时case公众号
            if(caseAppId.equalsIgnoreCase(authorizerAppId)) {//如果是全网case，回复特定消息
            	return initCaseMsg(outputMsg, inputMsg);
     ...
     * @return
     */
    private String initCaseMsg(OutputMessage outputMsg, InputMessage inputMsg) {
        ...
        try {
        	...
        	String outContent = "";
        	String caseContent = "TESTCOMPONENT_MSG_TYPE_TEXT";
        	if (Configuration.isDebug()) {
        		System.out.println("全网发布CASE：");
                System.out.println("全网发布CASE-inputContent：" + inputContent);
            }
        	if(caseContent.equalsIgnoreCase(inputContent)) 
        		outContent = "TESTCOMPONENT_MSG_TYPE_TEXT_callback";
        	else if(!Utils.isEmpty(inputContent) && inputContent.indexOf("QUERY_AUTH_CODE:") != -1) {
        		Weixin weixin = new Weixin();
        		String query_auth_code = inputContent.split(":")[1];
        		Authorization authorization =  weixin.caseAuthorization(query_auth_code);
        		String access_token = authorization.getAuthorization_info().getAuthorizer_access_token();
        		String openId = inputMsg.getFromUserName();
        		if (Configuration.isDebug()) {
                    System.out.println("全网发布CASE-query_auth_code：" + query_auth_code);
                    System.out.println("全网发布CASE-access_token：" + access_token);
                    System.out.println("全网发布CASE-openId：" + openId);
                }
        		weixin.customSendContent(access_token, openId, query_auth_code + "_from_api");
        		
        	}
        	else
        		outContent = inputMsg.getEvent().toUpperCase() + "from_callback";
        	
        	TextOutputMessage out = new TextOutputMessage();
            out.setContent(outContent);
			setOutputMsgInfo(out, inputMsg);
			// 把发送发送对象转换为xml输出
	        xml = out.toXML();
